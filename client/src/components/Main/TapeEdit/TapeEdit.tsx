...
import styles from "./TapeEdit.module.scss";
...
import { ITapeItem } from "src/models/Tape/ITapeModel";
...
const TapeEdit: FC<ITapeAddProps> = (props) => {
  const { dataTape, clearTape, clickEdit } = props;
  ...
  const onClickHandler = (tapes_id: number, title: string, content: ITapeContent[]) => {
    dispatch(UserTapeAction.editTape(tapes_id, title, content, () => {
      ...
      clickEdit && clickEdit();
      dispatch(messageQueueAction.addMessage(null, "success", "Лента успешно изменена!"));
    }));
  }
    ...
    <>
      <div className={styles.main}>
        ...
        <Button
          title={"Изменить"}
          clickHandler={() => {
            onClickHandler(dataTape.id, form.title, tapeSelector.items);
          }} />
      </div>
    </>
  );
